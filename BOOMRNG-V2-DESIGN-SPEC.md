...
**Implemented.** Enforcement pages now synchronously capture and validate the transient original-URL fragment into same-tab `sessionStorage`, and remove the fragment before any other page code runs — closing this section's steady-state address-bar exposure. This is not zero transient exposure: the fragment is still briefly present in the URL during the redirect handoff itself, for the short window before that capture-and-strip step completes.
