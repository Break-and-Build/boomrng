...
**What Milestone 0 establishes:** the underlying data needed to recognize this state — a constraint's `behavior === 'pin-required'` together with `settings.pin === null`, both already loadable via the same live-lookup mechanism in 30.1 — is available as a small, named, tested predicate rather than something each caller would otherwise re-derive inline. **What Milestone 0 explicitly does not do:** decide what the PIN enforcement page shows or does when it discovers this state. The current PIN page would simply present a PIN field that can never succeed (every candidate PIN necessarily fails the comparison against a `null` stored PIN) with no dead end otherwise — "Back to what I was doing" remains available, so this is a permanently-unopenable constraint, not a literal trap, but it is not a designed experience either. **This is an open UX decision for a later milestone**, not something this document or this implementation pass invents an answer for.
